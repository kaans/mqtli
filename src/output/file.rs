...
impl FileOutput {
    pub fn output(content: Vec<u8>, target_file: &OutputTargetFile) -> Result<(), OutputError> {
        match File::options()
            .append(!*target_file.overwrite()).truncate(*target_file.overwrite()).write(true).create(true).open(target_file.path()) {
            Ok(mut file) => {
                if target_file.prepend().is_some() {
                    if let Err(e) = file.write_all(target_file.prepend().clone().unwrap().as_bytes()) {
